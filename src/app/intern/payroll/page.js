...
export default function InternPayroll() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      console.log("[INTERN PAYROLL DIAGNOSTIC] Current Session:", session);
      if (session) {
        ...
        // Simulator check: If Admin is viewing intern portal for testing, show default intern stubs
        const emailToQuery = session.role === "Admin" ? "intern@hraconnect.com" : session.email;
        const data = await apiClient.getPayrolls(emailToQuery);
        console.log("[INTERN PAYROLL DIAGNOSTIC] Loaded Payslips for:", emailToQuery, data);
        setPayslips(data);
      } else {
        console.warn("[INTERN PAYROLL DIAGNOSTIC] No active session found!");
      }
      setIsLoading(false);
    }
    loadData();
    ...
    const htmlContent = `
      ...
      <body>
        ...
        <div class="title-section">
          <h1 class="slip-title">INTERN PAYSLIP</h1>
        ...
        <div class="info-grid">
          <div class="info-block">
            ...
            <div class="info-row">
              ...
              <span class="info-value">${user.designation || user.role || 'Intern'}</span>
            </div>
            <div class="info-row">
              <span class="info-label">Department</span>
              <span class="info-value">${user.department || 'Operation'}</span>
            </div>
          </div>
          <div class="info-block">
            <div class="info-row">
        ...
        <div class="net-pay-box">
          <div class="net-pay-label">NET STIPEND PAYABLE</div>
        ...
        <div class="footer-note">
          This is a system-generated stipend slip and does not require a physical stamp or signature.
        </div>
  ...
  const netPayStr = latestSlip ? `₹${formatCurrency(latestSlip.net)}` : "₹22,000.00";
  const deductionsStr = latestSlip ? `₹${formatCurrency(latestSlip.deductions)}` : "₹1,000.50";
  ...
  if (latestSlip) {
    const pfVal = latestSlip.pf !== undefined ? Number(latestSlip.pf) : Math.round(Number(String(latestSlip.deductions).replace(/[^\d.]/g, "")) * 0.00);
    const tdsVal = latestSlip.tds !== undefined ? Number(latestSlip.tds) : Math.round(Number(String(latestSlip.deductions).replace(/[^\d.]/g, "")) * 0.80);
  ...
  return (
    <div className="flex flex-col gap-8 text-left">
      ...
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">Intern Stipend & Compensation Console</h1>
        <p className="text-xs text-slate-500">Access your monthly itemized stipend slips, view taxes/deductions audits, and manage primary disbursement routes.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-gradient-to-br from-indigo-900 to-slate-950 text-white rounded-2xl p-6 shadow-md flex flex-col justify-between relative overflow-hidden">
          <div className="flex justify-between items-start z-10">
            <span className="text-[10px] font-bold text-indigo-200 uppercase tracking-wider">Net Take-Home Stipend</span>
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
          <div className="flex justify-between items-start">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Deductions (TDS & Tax)</span>
        ...
        {/* Annual fixed Stipend Card */}
        <div className="bg-white border border-slate-200/80 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
          <div className="flex justify-between items-start">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Annual Fixed Stipend</span>
          ...
          <div className="mt-8">
            <span className="text-2xl font-bold text-slate-950">₹3.0 Lakhs</span>
            <span className="block text-[10px] text-slate-400 font-semibold mt-1">Standard intern stipend bracket (L1 Grade)</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        ...
        <div className="lg:col-span-8 bg-white border border-slate-200/80 rounded-2xl shadow-sm overflow-hidden flex flex-col">
          <div className="border-b border-slate-150/75 px-6 py-4.5 bg-slate-50/40">
            <h3 className="font-bold text-slate-900 text-sm">Published Stipend Slips</h3>
          </div>
              ...
              <div key={slip.id} className="px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-50/30 transition-colors text-left">
                <div className="flex flex-col gap-0.5">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-slate-950 text-xs">Stipend Slip — {slip.id}</span>
            ...
            <form onSubmit={handleBankUpdate} className="space-y-5">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-slate-200 text-xs bg-white text-slate-955 font-bold focus:outline-none focus:border-indigo-400 transition-all"
                />
              </div>
