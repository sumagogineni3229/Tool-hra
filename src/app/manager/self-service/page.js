...
export default function ManagerSelfServiceApproval() {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const emailParam = `?email=${encodeURIComponent(currentUser.email)}&role=Manager`;
      const res = await fetch(`/api/ess-requests${emailParam}`);
      if (res.ok) {
  ...
  const handleAction = async (e) => {
    ...
    try {
      const payload = {
        ...
        reviewedBy: currentUser?.name || "Team Manager",
  ...
  return (
    <div className="space-y-12">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          <h2 className="text-4xl font-extrabold text-slate-905 tracking-tight mb-2">Team Self Service Approvals</h2>
          <p className="text-slate-500 font-light text-lg italic tracking-wide">Review and moderate WFH or WFO requests from your direct team members.</p>
        </div>
        <button
          onClick={fetchRequests}
          className="px-6 py-4 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-100 rounded-2xl font-black uppercase tracking-wider text-[10px] shadow-sm flex items-center gap-2 cursor-pointer transition-all active:scale-95"
        >
          <RefreshCw className={`w-4.5 h-4.5 ${loading ? "animate-spin" : ""}`} />
          Reload Team Requests
        </button>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 py-2">
        {[
          { label: "Team Applications", count: totalCount, icon: FileText, bg: "from-blue-50/50 to-indigo-50/10 border-blue-100/50 text-blue-600" },
          { label: "Pending Approvals", count: pendingCount, icon: ShieldAlert, bg: "from-amber-50/50 to-orange-50/10 border-amber-100/50 text-amber-600" },
          { label: "Approved Quota", count: approvedCount, icon: CheckCircle, bg: "from-emerald-50/50 to-teal-50/10 border-emerald-100/50 text-emerald-600" },
          { label: "Declined Protocol", count: rejectedCount, icon: XCircle, bg: "from-rose-50/50 to-red-50/10 border-rose-100/50 text-rose-600" },
        ].map((item, idx) => (
          <motion.div
            key={idx}
      ...
      <div className="bg-white border border-slate-100 p-10 rounded-[3.5rem] shadow-xl shadow-slate-200/30 space-y-10">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-6 pb-2 border-b border-slate-100/50">
          ...
          {/* Tabs */}
          <div className="flex bg-slate-50 border border-slate-100/80 p-1.5 rounded-2xl w-fit">
            {[
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              placeholder="Search team member, reason..."
        ...
        <div className="min-h-[250px] relative">
          {loading ? (
            <div className="py-24 text-center text-slate-450 font-black uppercase tracking-widest animate-pulse flex flex-col items-center justify-center gap-4">
              ...
              <span>Fetching Team Request Log...</span>
            </div>
          ) : activeTab === "pending" ? (
            <AnimatePresence mode="popLayout">
              {pending.length === 0 ? (
                ...
                >
                  ...
                  <p className="text-slate-400 font-light italic text-sm max-w-xs mx-auto">No team requests requiring active managerial review.</p>
                </motion.div>
              ) : (
                        ...
                        <div className="space-y-3 flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-3">
                            <span className="text-lg font-black text-slate-900 tracking-tight leading-none">
                              {req.userName || "Team Member"}
                            </span>
                            <span className="px-3 py-1 rounded-lg bg-emerald-50 text-emerald-800 border border-emerald-100/50 font-extrabold uppercase text-[9px] tracking-widest leading-none">
                          ...
                          <div className="flex flex-wrap items-center gap-4 text-slate-605 text-xs font-bold">
                          ...
                          <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl relative max-w-xl">
                            <p className="text-xs text-slate-505 font-light italic leading-relaxed">
            ...
            <AnimatePresence mode="popLayout">
              {processed.length === 0 ? (
                ...
                >
                  ...
                  <p className="text-slate-400 font-light italic text-sm">No processed requests inside this log.</p>
                </motion.div>
              ) : (
                <div className="space-y-6">
                  {processed.map((req) => {
                      ...
                      >
                        <div className="space-y-3 flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-3">
                            ...
                            <span className="px-2 py-0.5 rounded bg-emerald-50 text-emerald-805 font-extrabold uppercase text-[8px] tracking-widest leading-none">
                              {req.userRole || "Employee"}
                            </span>
                            <span className="text-[10px] text-slate-400 font-medium italic">
                              ({req.userEmail})
                          ...
                          {/* Dates and type */}
                          <div className="text-xs text-slate-650 font-semibold leading-none flex items-center gap-2">
                            <span className="text-emerald-600 font-bold">{cfg.label}</span>
                            <span>—</span>
                          ...
                          {/* Comments rationale */}
                          {req.managerComments && (
                            <div className="p-3 bg-emerald-50/30 border border-emerald-100/50 rounded-xl max-w-xl">
                              <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-1.5">
                                Manager Review Comments:
                              </p>
                              <p className="text-xs font-medium text-slate-705 leading-relaxed italic">
            ...
            <div className="border-b border-slate-100 px-8 py-5 bg-slate-50 flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mt-1">Manager Clearance Review</p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleAction} className="p-8 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full bg-slate-50 border border-slate-150 rounded-2xl px-4 py-3 focus:bg-white outline-none text-xs font-medium text-slate-805 leading-relaxed transition-all resize-none"
              ...
              <div className="flex justify-end gap-3 pt-3 border-t border-slate-100">
                <button
                  type="button"
                  onClick={() => setReviewRequest(null)}
                  className="px-5 py-2.5 rounded-xl text-xs font-bold text-slate-500 hover:text-slate-800 hover:bg-slate-100 transition-all cursor-pointer"
                >
                  Cancel
                </button>
                <button
