    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs text-slate-400 font-medium">Loading team reports...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1 font-medium">Review and approve daily work reports from your team</p>
        </div>
        <button onClick={loadData} className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-colors cursor-pointer">
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="Search employee, project..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                className="pl-8 pr-4 py-1.5 rounded-lg border border-slate-200 text-xs font-medium text-slate-600 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-100 w-52" />
            </div>
            {/* Status Filter */}
            <div className="relative">
        ...
        {filteredReports.length === 0 ? (
          <div className="py-16 flex flex-col items-center gap-3 text-center px-6">
            ...
            <p className="text-xs text-slate-400">Team members haven't submitted any reports yet.</p>
          </div>
        ) : (
                    ...
                    <tr key={id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="px-5 py-4">
                        <button onClick={() => openDetails(report)}
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-bold bg-emerald-50 text-emerald-700 border border-emerald-100 hover:bg-emerald-100 transition-colors cursor-pointer">
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <div className="flex items-start justify-between px-6 py-5 border-b border-slate-100 shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-base font-bold text-slate-900">{selectedReport.employeeName}</h2>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => setSelectedReport(null)} className="p-2 rounded-xl text-slate-400 hover:text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer"><X className="w-5 h-5" /></button>
                </div>
              </div>
              ...
              <div className="overflow-y-auto p-6 space-y-5 flex-1">
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="p-3 rounded-xl bg-emerald-50 border border-emerald-100 text-center">
                    ...
                    <p className="text-[10px] text-slate-500 font-semibold">Progress</p>
                  </div>
                ...
                {selectedReport.attachment?.name && (
                  <div className="flex items-center gap-2 p-3 rounded-xl bg-slate-50 border border-slate-100">
                    ...
                    <span className="text-xs font-semibold text-slate-600">{selectedReport.attachment.name}</span>
                  </div>
                )}
                ...
                <div>
                  ...
                  <textarea value={feedback} onChange={e => setFeedback(e.target.value)} rows={3}
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 text-xs font-medium text-slate-700 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-100 resize-none placeholder-slate-300" />
                </div>
              </div>
              ...
              <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 shrink-0 bg-slate-50/30">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => handleAction("feedback")} disabled={submitting || !feedback.trim()}
                    className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold border border-slate-200 text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer disabled:opacity-40">
