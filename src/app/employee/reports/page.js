...
const STATUS_CONFIG = {
  "Draft":          { color: "bg-slate-100 text-slate-600 border-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:border-slate-700",  icon: FileText,     dot: "bg-slate-400" },
  "Pending Review": { color: "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/30 dark:text-amber-400 dark:border-amber-900/50",   icon: Clock,        dot: "bg-amber-400" },
  "Approved":       { color: "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/30 dark:text-emerald-400 dark:border-emerald-900/50", icon: CheckCircle2, dot: "bg-emerald-400" },
  "Rejected":       { color: "bg-rose-50 text-rose-700 border-rose-200 dark:bg-rose-950/30 dark:text-rose-400 dark:border-rose-900/50",      icon: XCircle,      dot: "bg-rose-400" },
};
...
export default function EmployeeReportsPage() {
  ...
  const handleSubmit = async (submitStatus) => {
    ...
    try {
      ...
      let result;
      if (editingReport) {
        result = await apiClient.updateReport({ id: editingReport._id || editingReport.id, ...payload });
      } else {
        result = await apiClient.submitReport(payload);
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <button
          onClick={openNewForm}
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-rose-600 text-white text-sm font-bold hover:bg-rose-700 shadow-md hover:shadow-lg transition-all duration-150 cursor-pointer"
        >
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              <select
                ...
                className="text-xs font-semibold text-slate-600 border border-slate-200 rounded-lg px-3 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-rose-100 appearance-none pr-7 cursor-pointer"
              >
            ...
            <button onClick={loadData} className="p-1.5 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors cursor-pointer" title="Refresh">
        ...
        {filteredReports.length === 0 ? (
          <div className="py-16 flex flex-col items-center gap-3 text-center px-6">
            ...
            <p className="text-xs text-slate-400">Submit your first daily work report to get started.</p>
            <button onClick={openNewForm} className="mt-2 px-4 py-2 rounded-xl bg-rose-600 text-white text-xs font-bold hover:bg-rose-700 transition-colors cursor-pointer">
              Submit Report
            </button>
          </div>
        ) : (
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          <div className="w-16 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                            <div className="h-full rounded-full bg-rose-400" style={{ width: `${report.progressPercentage || 0}%` }} />
                          </div>
                      ...
                      <td className="px-5 py-4 max-w-[160px]">
                        {report.managerFeedback ? (
                          <span className="flex items-center gap-1 text-indigo-600 font-semibold">
                            ...
                            <span className="truncate">{report.managerFeedback}</span>
                          </span>
                        ) : <span className="text-slate-300 italic">—</span>}
                      </td>
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-2">
                          <button onClick={() => setViewReport(report)} className="p-1.5 rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors cursor-pointer" title="View">
                            <Eye className="w-3.5 h-3.5" />
                          </button>
                          {canEdit && (
                            <button onClick={() => openEditForm(report)} className="p-1.5 rounded-lg text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 transition-colors cursor-pointer" title="Edit">
                              <Edit3 className="w-3.5 h-3.5" />
                            </button>
                          )}
                          {report.status === "Draft" && (
                            <button onClick={() => handleDelete(id)} className="p-1.5 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 transition-colors cursor-pointer" title="Delete">
            ...
            >
              ...
              <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 shrink-0">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">Fill in your work details for today</p>
                </div>
                <button onClick={() => setFormOpen(false)} className="p-2 rounded-xl text-slate-400 hover:text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="overflow-y-auto p-6 space-y-5 flex-1">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input type="date" value={form.reportDate} onChange={e => setForm(p => ({ ...p, reportDate: e.target.value }))}
                        className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-slate-200 text-xs font-medium text-slate-700 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-rose-100 focus:border-rose-200" />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <select value={form.projectId} onChange={handleProjectChange}
                        className="w-full pl-9 pr-7 py-2.5 rounded-xl border border-slate-200 text-xs font-medium text-slate-700 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-rose-100 appearance-none cursor-pointer">
                ...
                <div>
                  ...
                  <textarea value={form.workCompleted} onChange={e => setForm(p => ({ ...p, workCompleted: e.target.value }))} rows={4}
                    ...
                    className="w-full px-4 py-3 rounded-xl border border-slate-200 text-xs font-medium text-slate-700 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-rose-100 resize-none placeholder-slate-300" />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {/* Progress */}
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">Progress Percentage: <span className="text-rose-600">{form.progressPercentage}%</span></label>
                  ...
                  <div>
                    ...
                    <input type="number" min={0} max={24} value={form.hoursWorked}
                      ...
                      className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs font-medium text-slate-700 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-rose-100" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <label className="flex items-center gap-3 px-4 py-3 rounded-xl border-2 border-dashed border-slate-200 hover:border-rose-200 hover:bg-rose-50/30 transition-colors cursor-pointer group">
                    <Paperclip className="w-4 h-4 text-slate-400 group-hover:text-rose-500 transition-colors" />
              ...
              <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100 shrink-0 bg-slate-50/30">
                <button onClick={() => setFormOpen(false)} className="px-4 py-2.5 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-100 transition-colors cursor-pointer">
                  Cancel
                </button>
                ...
                <button onClick={() => handleSubmit("Pending Review")} disabled={submitting}
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold bg-rose-600 text-white hover:bg-rose-700 shadow-md transition-all cursor-pointer disabled:opacity-50">
            ...
            >
              <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 shrink-0">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={() => setViewReport(null)} className="p-2 rounded-xl text-slate-400 hover:text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer">
                    <X className="w-5 h-5" />
                  </button>
                </div>
              </div>
              <div className="overflow-y-auto p-6 space-y-4 flex-1">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-100 text-center">
                    <p className="text-lg font-bold text-rose-600">{viewReport.progressPercentage || 0}%</p>
              ...
              <div className="flex justify-end gap-3 px-6 py-4 border-t border-slate-100 shrink-0">
                ...
                <button onClick={() => setViewReport(null)} className="px-5 py-2.5 rounded-xl text-xs font-bold bg-slate-900 text-white hover:bg-slate-800 transition-colors cursor-pointer">
                  Close
                </button>
              </div>
            </motion.div>
