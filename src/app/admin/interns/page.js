...
export default function AdminInternsDashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      if (reportsRes.ok) {
        ...
        setReports(rData?.reports || (Array.isArray(rData) ? rData : []));
      }
      ...
      if (projectsRes.ok) {
        ...
        setProjects(pData?.projects || (Array.isArray(pData) ? pData : []));
      }
    } catch (err) {
  ...
  const triggerViewAttendance = async (intern) => {
    ...
    try {
      const res = await fetch(`/api/attendance?email=${encodeURIComponent(intern.email)}&t=${Date.now()}`, { cache: 'no-store' });
      if (res.ok) {
